...
if NOT [%3]==[] set SOLARFRAMEWORKROOT=%3
...
if exist %SOLARFRAMEWORKROOT%/installpackages.txt (
...
%QMAKE_PATH%\qmake.exe ../../../../../%SOLARFRAMEWORKROOT%/%PROJECT%.pro -spec win32-msvc CONFIG+=debug CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe
::%JOM_PATH%\jom.exe install

cd %~dp0\build\core\%PROJECT%\shared\release
%QMAKE_PATH%\qmake.exe ../../../../../%SOLARFRAMEWORKROOT%/%PROJECT%.pro -spec win32-msvc CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe
::%JOM_PATH%\jom.exe install
...
echo.
echo Usage: params [Qt kit version to use - default='%QTVERSION%'] [Visual version to use - default='%VISUALVERSION%'] [path to SolAR Framework project root - default='%SOLARFRAMEWORKROOT%'] 
