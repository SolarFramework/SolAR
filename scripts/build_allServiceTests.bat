...
if not exist %SOLARROOTFOLDER%\SolARAllServiceTests.pro (
    echo "QT project SolARAllServiceTests.pro doesn't exist in folder %SOLARROOTFOLDER%"
    goto:eof
)

echo "SOLAR all service tests QT project used is : %SOLARROOTFOLDER%\SolARAllServiceTests.pro"

for /f "delims=" %%A in ('powershell -Command "& {Select-String -Path "%SOLARROOTFOLDER%\SolARAllServiceTests.pro" -Pattern 'services.*pro' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value}"') do (call:process_service_test_path %%A)
endlocal
goto:eof

:process_service_test_path
set "serviceTestPath=%~1"
    
set "_slash_pos="
set "noslash=%serviceTestPath:/=%"
if "%noslash%"=="%serviceTestPath%" goto :jjloopEnd
set /a jj=1
:jjloop
call set "_auxiliary=%%serviceTestPath:~-%jj%%%"
...
if not defined _slash_pos (
    ...
    set "vara=%serviceTestPath%"
    set "varb="
    goto :launchBuild
)
call set "vara=%%serviceTestPath:~0,-%_slash_pos%%%"
if %_slash_pos% EQU 1 (
    set "varb="
) else (
    call set "varb=%%serviceTestPath:~-%_slash_pre%%%"
)
:launchBuild
...
:buildAndInstall
echo "Build service test %~3 found in %~1\%~2"
...
if exist build\servicetests\%~3\shared rmdir /S /Q build\servicetests\%~3\shared
if not exist build\servicetests\%~3\shared\debug mkdir build\servicetests\%~3\shared\debug
if not exist build\servicetests\%~3\shared\release mkdir build\servicetests\%~3\shared\release

cd %~dp0\build\servicetests\%~3\shared\release
%QMAKE_PATH%\qmake.exe ..\..\..\..\..\%~1\%~2\%~3.pro -spec win32-msvc CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe

cd %~dp0\build\servicetests\%~3\shared\debug
...
echo This script builds service tests.
