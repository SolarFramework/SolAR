...
if not exist %SOLARROOTFOLDER%\SolARAllPipelineTests.pro (
    echo "QT project SolARAllPipelineTests.pro doesn't exist in folder %SOLARROOTFOLDER%"
    goto:eof
)

echo "SOLAR all pipeline tests QT project used is : %SOLARROOTFOLDER%\SolARAllPipelineTests.pro"

for /f "delims=" %%A in ('powershell -Command "& {Select-String -Path "%SOLARROOTFOLDER%\SolARAllPipelineTests.pro" -Pattern 'samples.*pro' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value}"') do (call:process_pipeline_test_path %%A)
endlocal
goto:eof

:process_pipeline_test_path
set "pipelineTestPath=%~1"
    
set "_slash_pos="
set "noslash=%pipelineTestPath:/=%"
if "%noslash%"=="%pipelineTestPath%" goto :jjloopEnd
set /a jj=1
:jjloop
call set "_auxiliary=%%pipelineTestPath:~-%jj%%%"
...
if not defined _slash_pos (
    ...
    set "vara=%pipelineTestPath%"
    set "varb="
    goto :launchBuild
)
call set "vara=%%pipelineTestPath:~0,-%_slash_pos%%%"
if %_slash_pos% EQU 1 (
    set "varb="
) else (
    call set "varb=%%pipelineTestPath:~-%_slash_pre%%%"
)
:launchBuild
...
:buildAndInstall
echo "Build pipeline test %~3 found in %~1\%~2"
...
if exist build\pipelineTests\%~3\shared rmdir /S /Q build\pipelineTests\%~3\shared
if not exist build\pipelineTests\%~3\shared\debug mkdir build\pipelineTests\%~3\shared\debug
if not exist build\pipelineTests\%~3\shared\release mkdir build\pipelineTests\%~3\shared\release

cd %~dp0\build\pipelineTests\%~3\shared\release
%QMAKE_PATH%\qmake.exe ..\..\..\..\..\%~1\%~2\%~3.pro -spec win32-msvc CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe

cd %~dp0\build\pipelineTests\%~3\shared\debug
...
echo This script builds pipeline tests.
