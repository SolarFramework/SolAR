...
if not exist %SOLARROOTFOLDER%\SolARAllSamples.pro (
    echo "QT project SolARAllSamples.pro doesn't exist in folder %SOLARROOTFOLDER%"
    goto:eof
)

echo "SOLAR all samples QT project used is : %SOLARROOTFOLDER%\SolARAllSamples.pro"

for /f "delims=" %%A in ('powershell -Command "& {Select-String -Path "%SOLARROOTFOLDER%\SolARAllSamples.pro" -Pattern 'samples.*pro' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value}"') do (call:process_sample_path %%A)
endlocal
goto:eof
...
set "_slash_pos="
set "noslash=%samplePath:/=%"
if "%noslash%"=="%samplePath%" goto :jjloopEnd
set /a jj=1
:jjloop
call set "_auxiliary=%%samplePath:~-%jj%%%"
...
call set "vara=%%samplePath:~0,-%_slash_pos%%%"
if %_slash_pos% EQU 1 (
    set "varb="
) else (
    call set "varb=%%samplePath:~-%_slash_pre%%%"
)
:launchBuild
...
:buildAndInstall
echo "Build sample %~3 found in %~1\%~2"
...
if exist build\samples\%~3\shared rmdir /S /Q build\samples\%~3\shared
if not exist build\samples\%~3\shared\debug mkdir build\samples\%~3\shared\debug
if not exist build\samples\%~3\shared\release mkdir build\samples\%~3\shared\release

cd %~dp0\build\samples\%~3\shared\release
%QMAKE_PATH%\qmake.exe ..\..\..\..\..\%~1\%~2\%~3.pro -spec win32-msvc CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe

cd %~dp0\build\samples\%~3\shared\debug
...
echo This script builds samples.
