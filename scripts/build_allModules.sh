...
modules=("SolARModuleFBOW,SolARModuleFBOW" "SolARModuleG2O,SolARModuleG2O" "SolARModuleOpenCV/SolARModuleOpenCV,SolARModuleOpenCV" "SolARModuleOpenGL,SolARModuleOpenGL" "SolARModuleTools,SolARModuleTools" "SolARModuleHapPLY,SolARModuleHapPLY")
modulesAndroid=("SolARModuleFBOW, SolARModuleFBOW" "SolARModuleG2O,SolARModuleG2O" "SolARModuleOpenCV/SolARModuleOpenCV,SolARModuleOpenCV" "SolARModuleTools,SolARModuleTools")

display_usage() { 
	echo "This script builds the SolAR modules in shared mode."
    	echo "It can receive four optional arguments." 
	echo -e "\nUsage: \$0 [ Cross build ? | default='${CROSSBUILD}' | possible values=[NO, ANDROID]] [Nb processors used for building | default='${NBPROCESSORS}'] [Qt kit version to use | default='${QTVERSION}'] [path to SolAR modules root folder | default='${SOLARMODULESROOT}'] \n" 
}
...
BUILDREPORT=""
if [ -f build/${PLATEFORMFOLDER}modules/report.txt ]; then
	rm -f build/${PLATEFORMFOLDER}modules/report.txt
fi


buildAndInstall() {

if [ -d build/${PLATEFORMFOLDER}modules/${2}/shared ]; then
	rm -rf build/${PLATEFORMFOLDER}modules/${2}/shared
fi
mkdir -p build/${PLATEFORMFOLDER}modules/${2}/shared/debug
mkdir -p build/${PLATEFORMFOLDER}modules/${2}/shared/release

echo "===========> run remaken for ${2} <==========="
