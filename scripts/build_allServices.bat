...
if not exist %SOLARROOTFOLDER%\SolARAllServices.pro (
    echo "QT project SolARAllServices.pro doesn't exist in folder %SOLARROOTFOLDER%"
    goto:eof
)

echo "SOLAR all services QT project used is : %SOLARROOTFOLDER%\SolARAllServices.pro"

for /f "delims=" %%A in ('powershell -Command "& {Select-String -Path "%SOLARROOTFOLDER%\SolARAllServices.pro" -Pattern 'services.*pro' | Select-Object -ExpandProperty Matches | Select-Object -ExpandProperty Value}"') do (call:process_service_path %%A)
endlocal
goto:eof
...
set "_slash_pos="
set "noslash=%servicePath:/=%"
if "%noslash%"=="%servicePath%" goto :jjloopEnd
set /a jj=1
:jjloop
call set "_auxiliary=%%servicePath:~-%jj%%%"
...
if not defined _slash_pos (
    ...
    set "vara=%servicePath%"
    set "varb="
    goto :launchBuild
)
call set "vara=%%servicePath:~0,-%_slash_pos%%%"
if %_slash_pos% EQU 1 (
    set "varb="
) else (
    call set "varb=%%servicePath:~-%_slash_pre%%%"
)
:launchBuild
...
:buildAndInstall
echo "Build service %~3 found in %~1\%~2"
...
if exist build\services\%~3\shared rmdir /S /Q build\services\%~3\shared
if not exist build\services\%~3\shared\debug mkdir build\services\%~3\shared\debug
if not exist build\services\%~3\shared\release mkdir build\services\%~3\shared\release

cd %~dp0\build\services\%~3\shared\release
%QMAKE_PATH%\qmake.exe ..\..\..\..\..\%~1\%~2\%~3.pro -spec win32-msvc CONFIG+=qml_debug && %JOM_PATH%\jom.exe qmake_all
%JOM_PATH%\jom.exe

cd %~dp0\build\services\%~3\shared\debug
...
echo This script builds services.
